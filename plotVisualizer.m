...
% with George's idea, with probability, no re-iteration, but more careful
% with tau !!  --> looking in the past AND future, 
% with Naomi' idea of integrating the distance to the circles if circles 
% do not intersect. Identify areas of switching by comparison
...
handles.adjacencyMatrix = adjacencyMatrix;
 ...
 if inputEdgeChoice==4
  ...
  %disp('Clock 00: before reading all the data')
  %clock
  ...
  %disp('Clock 01: after reading all the data')
  %clock
  ...
  for o = 1:numberOfNodes
   ...
   while i <= ub-tau2   
     ...
     %disp('Clock 1: beginning the loop time i=lb:ub-tau2')
     %clock
     
     distanceMatrix = ipdm(squeeze(positionMatrix(i-lb+1,:,:)));
    
     %{
     z1 = size(find(adj3{i}(o,:)==2),2);
     z2 = size(find(adj3{i}(o,:)==1),2);
     z3 = size(find(adj3{i}(o,:)==0),2);
      %}
     ...
     disp('pos_h and posInd')
     posInd
     pos_h
     %{    
     if size(find(adj3{i}(o,:)==2),2)>=numberOfNeighbors
         potential_neighbors = find(adj3{i}(o,:)==2);
         pn = z1;
        
     elseif size(length(find(adj3{i}(o,:)==2)) + length(find(adj3{i}(o,:)==1)),2)>=numberOfNeighbors
         potential_neighbors = [find(adj3{i}(o,:)==2), find(adj3{i}(o,:)==1)];       
       
         pn = [z1,z2];
         pn(find(pn==0))=[];   
     else
     %}
         %potential_neighbors = [find(adj3{i}(o,:)==2), find(adj3{i}(o,:)==1), find(adj3{i}(o,:)==0)];
...
closest_neighbors
     ...
     %end
     %{
     [ignore, distSorted_ind] = sort(distanceMatrix(o,:));  
     
     switch length(pn)
         case 1
          closest_neighbors = potential_neighbors(1:numberOfNeighbors);
          
         case 2

     ind1=[];        
     if z1~=0
        p1 = ismember(distSorted_ind, potential_neighbors(1:z1));
        ind1 = distSorted_ind(find(p1(1:6)==1));
     end
     
       ind2=[];
     if z2~=0
        p2 = ismember(distSorted_ind, potential_neighbors(z1+1:z1+z2));
        ind2= distSorted_ind(find(p2(1:6)==1));
     end
     
     ind = [ind1, ind2]; 
     
     if size(length(find(adj2{i}(o,:)==2)) + length(find(adj2{i}(o,:)==1)),2)<numberOfNeighbors
      if length(ind)<=numberOfNeighbors
        pp = numberOfNeighbors-length(ind);  
        p3=find(distSorted_ind, potential_neighbors(z1+z2+1:z1+z2+z3));
        ind3 = distSorted_ind(find(p3==1)); ind3 = ind3(1:pp);
        ind = [ind, ind3];
      end
     end
     
     closest_neighbors = ind(1:numberOfNeighbors);
       ...
       if length(ind)<=numberOfNeighbors
        pp = numberOfNeighbors-length(ind);  
        p3=ismember(distSorted_ind, potential_neighbors(z1+z2+1:z1+z2+z3));
        ind3 = distSorted_ind(find(p3==1)); ind3 = ind3(1:pp);
        ind = [ind, ind3];
       end
     end
 ...
 %choose closest neighbors
     ...
     distanceRow = distanceMatrix(o,:);
            distanceRow(o) = Inf;
            [organizedDistanceRow, closest_neighbors] = sort(distanceRow);
            closest_neighbors = closest_neighbors(1:numberOfNeighbors);
     %}       
%{ 
            if o==chosen
                i
  ...
  %}          
             ...
             if k==0
                ...
                for f = 1:numberOfNeighbors-1
                  for g = f+1:numberOfNeighbors
                      %'lol'
                      %pdist([pos_x(j,closest_neighbors(f)),pos_y(j,closest_neighbors(f)); pos_x(j,closest_neighbors(g)),pos_y(j,closest_neighbors(g))])
                      %3*inputRadius
                      ...
                      if pdist([pos_x(j,closest_neighbors(f)),pos_y(j,closest_neighbors(f)); pos_x(j,closest_neighbors(g)),pos_y(j,closest_neighbors(g))]) < 3*inputRadius  %if these are too far apart, we do not consider them
                        ...
                        if closest_neighbors(f)<closest_neighbors(g)  
                         ind_pos_neigh(sum_cnt,:) = [closest_neighbors(f), closest_neighbors(g)];
                        else
                         ind_pos_neigh(sum_cnt,:) = [closest_neighbors(g), closest_neighbors(f)];   
                        end
          ...
          %{     
           ...
           %if sum_cnt ==0, so no neighbors close to each other -- TBD
           
          %integrate the distances
          for h = 1:sum_cnt
                   
                   [int1, int2] = circle_and_int([pos_x(j,ind_pos_neigh(h,1)), pos_y(j, ind_pos_neigh(h,1))], [pos_x(j,ind_pos_neigh(h,2)), pos_y(j,ind_pos_neigh(h,2))], inputRadius); %intersection of circles around the 2 neighbors
                   int1 = double(int1);
                   int2 = double(int2);
         ...
         %{   
            disp('Clock 2: after the k loop')
            clock
           %} 
            sum_tau2=sum_tau;
            %find the minimums
            [min_dist, min_dist_ind] = min(sum_tau);
            potential_neighbors = ind_pos_neigh(min_dist_ind,1:2);
            potential_neighbors = sort(squeeze(potential_neighbors)');
            
            if o==chosen
             disp('potential neighbors')
             potential_neighbors
            end
         %{   
            if o==chosen
              disp('sum_tau')
              disp('potential neighbors at frame i')
              potential_neighbors
            end
            %}
           
           % until here all's alright...
            ...
            old_neighbors = sort(graphProperties(i-1).neighbors(o,:));
            potential_neighbors = squeeze(potential_neighbors)';
            
            %{
            if o==chosen
                 disp('old & potential neighbors')
                 old_neighbors
                 potential_neighbors
            end
            %}
            ...
            if sum(eq(old_neighbors, potential_neighbors)) == size(old_neighbors,2) %if old neighbors are the same as new predicted ones, then change to old neighbor
             ...
             if o==chosen  
               disp('II. were in the case when old neighbors = potential')
               neighbors
               disp('case II. how neigh_data changes')
               squeeze(neighbor_data(o, 1, cnt(o)))
               squeeze(neighbor_data(o, 2, cnt(o))) 
             end   
              
                
            else
                
               for k = tau1:tau2 %if not, enlarge window
          ...
          for h = 1:sum_cnt
              ...
              %{ 
               disp('Clock 3: after the k loop')
               clock 
               %}
       
               [min_dist, min_dist_ind] = min(sum_tau);
               potential_neighbors2 = ind_pos_neigh(min_dist_ind,1:2);
               potential_neighbors2 = sort(squeeze(potential_neighbors2));
             
                   if o==chosen
                   disp('potential_neighbors2');
                   potential_neighbors2
                   end
           %}    
               ...
               if sum(eq(potential_neighbors, potential_neighbors2)) == size(potential_neighbors2,2) 
                  ...
                  if sum(old_neighbors==0)==length(old_neighbors)
                   ...
                   if o==chosen
                       disp('switch pt = i b/c old_neighbors==[00]')
                       switch_pt
                   end
                   
                  else
                     % disp('clock 4: before switch_pt alg')
                     % clock
                   ...
                   if o==chosen
                     ...
                     % disp('clock 5: after switch algorithm')
                     % clock
                  end
                  ...
                  for b=switch_pt:i+tau1-1 
                   ...
                   if i>=switch_pt
                       ...
                       if neighbor_data(o,2,cnt(o)-1)<=0
                          ...
                          if o==chosen 
                           disp('were in weird case no. 1: when neighbor_data(o,2,cnt(o)-1)<=0')
                           i
                           switch_pt
                           neighbor_data(o,1,cnt(o)-1)
                           neighbor_data(o,2,cnt(o)-1)
                           neighbor_data(o,1,cnt(o))
                           neighbor_data(o,2,cnt(o))
                          end
                       ...
                       if neighbor_data(o, 2,cnt(o))<=0
                           ...
                           if o==chosen
                               disp('were in weird case no. 2: neighbor_data(o, 2,cnt(o))<=0')
                               i
                               switch_pt
                               neighbor_data(o,1,cnt(o))
                               neighbor_data(o,2,cnt(o))
                  ...
                  if o==chosen 
                      disp('III. switch point, case potential_neigh==potential_neigh2')
                      switch_pt
                      i
                      neighbors
                 
                      disp('case III how neigh_data changes')
                     if cnt(o)>1 
                      squeeze(neighbor_data(o,1,cnt(o)-1))
                      squeeze(neighbor_data(o,2,cnt(o)-1))
                     end
                      squeeze(neighbor_data(o,1,cnt(o)))
                      squeeze(neighbor_data(o,2,cnt(o)))
                  end
                  ...
                  %disp('Clock 6: after potential_neighbors==potential_neighbors2')
                  %clock
                  
               else  %when in the long run not the main pair (tau1 ~= tau2)
                   
                if sum(eq(potential_neighbors2, old_neighbors)) == size(old_neighbors,2)
                  ...
                  %}     
                       if ok==1 & sum_tau2(ind_neighbor)/tau1 < 0.4
                        ...
                        if o==chosen  
                          disp('ok==1 & sum_tau<0.4. deci cazul cand old_neighbors==potential_neighbors2 si old_neighbors e pe lista apropiatilor')  
                          i
                          neighbors
                        end
                        %}
                          ...
                          if o==chosen 
                            disp('IV. case when old_neigh == potential_neigh2 and sum not good so inconclusive')
                            i
                            neighbors
                          
                            disp('case IV how neigh_data changes')
                            squeeze(neighbor_data(o, 1, cnt(o)))
                            squeeze(neighbor_data(o, 2, cnt(o)))
                     ...
                     if o==chosen  
                       disp('V. we are in the case when all are different and so inconclusive')
                       squeeze(neighbor_data(o, 1, cnt(o)))
                       squeeze(neighbor_data(o, 2, cnt(o)))
                       i
                     end
   ...
   %disp('Clock 7: done with 1 node! and roughly the adj matrix')
   %ck
   
  end
  
%go back... and modify wherever needed.

adjacencyMatrix = zeros(ub-tau2-lb+1, numberOfNodes, numberOfNodes);
neighborNumber = zeros(1, numberOfNodes);
neighborNumber2 = zeros(1, numberOfNodes);

jj = zeros(1, numberOfNodes);
...
for o=1:numberOfNodes
    ...
    if size(final,1)==0
        final = size(neighbor_data(o,1,:),1);
    else
        final = final-1;
    end
    ...
    while j<=final
        ...
        if sum(eq(squeeze(neighbor_data1(o,j,:))',[0 0]))~=2  % when neighbors are not [0 0]
            
            jj(o)=jj(o)+1;
            neighbor_data3(o,:,jj(o))=neighbor_data(o,1:2,j);
           ...
           if j<final %when neighbors are [0 0], but are not at the end
            ...
            elseif neighbor_data(o,2,j)<=tau2
...
for o=1:numberOfNodes
    ...
    final = find(neighbor_data(o,1,:)==0,1);
    if size(final,2)==0
        final = size(neighbor_data(o,1,:),2);
    else
        final = final-1;
    end
    ...
    for j = 1:final    
       ...
       if neighbor_data(o,2,j)<=tau2 %if it's too short
            
         if j==1  %if it's the neighbor at the first frame
             
             if neighbor_data(o,2,2)>=tau2 & neighbor_data(o,3,2)==0 %if the next neighbors are along for more than tau2 and conclusive
                 
                neighbors = graphProperties(neighbor_data(o,1,1)).neighbors(o,:);
                
              for b = lb:lb+neighbor_data(o,2,1)-1 
            
                adjacencyMatrix(b,o,:) = zeros(1,1,numberOfNodes);
                adjacencyMatrix(b,o,neighbors) = 1;
                graphProperties(b).neighbors(o,:) = neighbors; 
              end
              ...
              if final>=3
             ...
             else  %if next neighbors are <tau2 or inconclusive
                    
                i=neighbor_data(o,1,2);
       
                      file=strcat('Dance',num2str(j),'.dat');
                      data_type = handles.data_type;
                      if data_type==1
                        data=handles.dancedata{j};
                        set(handles.dataselection_editText,'String',j);
                        pos_x = data(:,1);
                        pos_y = data(:,2);
                        theta = data(:,3);
                        [pos_x pos_y] = transf_in_familiar_coord(pos_x,pos_y,theta); 
                        positionMatrix = horzcat(pos_x,pos_y);
                      end
                  ...
                  for l=1:numberOfNeighbors
                     pos(l,:) =[pos_x(closest_neighbors(l)),pos_y(closest_neighbors(l))];   %position of neighbors
                  end
          
                  pos_node = [x(o), y(o)];            %position of node in question
                  
                  ind_pos_neigh = [];
                  sum_cnt = 1;
                for f = 1:numberOfNeighbors-1
                  for g = f+1:numberOfNeighbors
                      if pdist([pos(f,:); pos(g,:)]) < 3*inputRadius
                ...
                sum_cnt = sum_cnt-1; % how many pairs of neighbors are eventually considered  
                
                pairOfNodes = squeeze(ind_pos_neigh(:,1,1:2));
               if size(pairOfNodes,2)==1
                pairOfNodes = pairOfNodes';
               end
          
                [sum_to_integrate, winner] =  integrate(handles, lb, neighbor_data(o,2,1)+neighbor_data(o,2,2), pairOfNodes, o, inputRadius);
                neighbors = winner;
                   
                       for i=neighbor_data(o,1,1):neighbor_data(o,1,1)+neighbor_data(o,2,1)+neighbor_data(o,2,2)-1   
                    
                            graphProperties(i).neighbors(o,:) = neighbors;
                            adjacencyMatrix(i,o,:) = zeros(1,1,numberOfNodes);
                            adjacencyMatrix(i,o,neighbors) = 1;
                       end 
            ...
            neighbors_before = graphProperties(neighbor_data(o,1,j)-1).neighbors(o,:);
           if j~=final & neighbor_data(o,3,j+1)~=1
            neighbors_after = graphProperties(neighbor_data(o,1,j)+neighbor_data(o,2,j)).neighbors(o,:);
           end
           
            if neighbor_data(o,2,j) <= tau3
                   casus = 0;   
            else if j~=final
                  if neighbor_data(o,2,j+1)>=tau2 & neighbor_data(o,3,j+1)==0
                   casus = 1;
                  else
                   casus = 2;  
                  end
           ...
           if casus == 0 %if length < tau3, then change all to the previous neighbors  
               
             neighbors = neighbors_before;
             for i=neighbor_data(o,1,j):neighbor_data(o,1,j)+neighbor_data(o,2,j)-1
                graphProperties(i).neighbors(o,:) = neighbors;
                adjacencyMatrix(i,o,:) = zeros(1,1,numberOfNodes);
                adjacencyMatrix(i,o,neighbors) = 1;
             end
           ...
           if casus == 1 & sum(neighbors_before == neighbors_after) == sum(neighbors_after,2) %both before and after it has neighbors and they're the same
                 ...
                 if casus==1
                    pairOfNodes = [neighbors_before; neighbors_after];
                 else
                   pairOfNodes = [neighbors_before];
                 end
                 ...
                 if size(pairOfNodes,2)==1
                  ...
                  [sum_to_integrate, winner] =  integrate(handles, neighbor_data(o,1,j), neighbor_data(o,2,j), pairOfNodes, o, inputRadius);
                  sum_to_integrate = sum_to_integrate/neighbor_data(o,2,j);
                  
                  if sum_to_integrate > 0.5  %can change this limit; this is some parameter
                                             %if distance to neighbors is too large 
                      i=neighbor_data(o,1,j);
                ...
                [sum_to_integrate2, winner2] =  integrate(handles, neighbor_data(o,1,j), neighbor_data(o,2,j), pairOfNodes, o, inputRadius);
                
                if sum_to_integrate2 < sum_to_integrate
                    neighbors = winner2;
                else
                    neighbors = winner;
                end
           ...
           if neighbor_data(o,2,j)>=tau2 & neighbor_data(o,3,j) == 1 %if inconclusive for a longer time
              ...
              i=neighbor_data(o,1,j);
                ...
                for i=neighbor_data(o,1,j):neighbor_data(o,1,j)+neighbor_data(o,2,j)-1   
                   ...
                   graphProperties(i).neighbors(o,:) = neighbors;
                   adjacencyMatrix(i,o,:) = zeros(1,numberOfNodes);
                   adjacencyMatrix(i,o,neighbors) = 1;
                end
           end 
    end
...
save('NewModel2_r03_N4.mat','adjacencyMatrix', 'neighbor_data', 'neighbor_data1', 'neighbor_data3', 'neighbor_data4')
...
function [switch_pt] = determine_switch(handles, pt, duration, neighbors1, neighbors2, node, tau1, inputRadius)
        ...
        sum_tau =[0 0];
        duration2 = min(duration, floor(tau1/2));
        
           for i = pt-duration2:pt+floor(tau1/2)
...
% changing the order of the points so that it matches what I previously considered; optional, not needed    
xx(1) = x(6);
xx(2) = x(12);
xx(3) = x(9);
xx(4) = x(13);
...
ub = handles.ub;
    ...
    [ignore, ignore, ignore, center_avg(:,1), ignore] = averaging(center(:,1),delay,1,1);
    [ignore, ignore, ignore, center_avg(:,2), ignore] = averaging(center(:,2),delay,1,1);
    
    for i=start_frame:end_frame-delay
            center2(i-start_frame+1,:) = center(i-start_frame+1+delay,:)-center(i-start_frame+1,:);
            dir_center(i-start_frame+1) = atan2(center2(i-start_frame+1,2), center2(i-start_frame+1,1));
          for j=1:numberOfNodes  
...
function [positionMatrix, distanceMatrix, avg_distance, avg_distance_inTime, avg_distance_inTime_closest1, avg_distance_inTime_closest2, avg_distanceAll, avg_dist_index, closest_neighbors, relativeAngles, HeadingDiff, angleToclosest, Avg_angleToclosest, Avg_angleOverall] = computeAll_pos_dist(handles, start, ending, option)
  ...
  if option==1  
    for j=1:numberOfNodes
         angleToclosest(i,j,1) = anglerestrict(theta(j) - relativeAngles(i,j,closest_neighbors(i,j,1)))*180/pi; %to closest node
         angleToclosest(i,j,2) = anglerestrict(theta(j) - relativeAngles(i,j,closest_neighbors(i,j,2)))*180/pi; % to second closest node
         angleToclosest(i,j,3) = (angleToclosest(i,j,1)+angleToclosest(i,j,2))/2; %mean between the two
...
for i=1:end_frame-start_frame+1
 ...
 for i = start_frame:end_frame-20
   ...
   if n==1   
     ...
     Distance3(i-start_frame+1,:) = Distance3(i-start_frame+1,:)/max_Distance3;
 ...
 for j=1:N
     ...
     Distance2_mean(j) = mean(Distance2(:,j));
     Distance2_mean(j)
     mean(Distance2(j,Distance2(j,:)>=0))
     mean(Distance2(j,Distance2(j,:)<=0))
 end
 
 figure(1)
 bar(Distance2_mean)
 [ignore, ind_Distance2_mean] = sort(Distance2_mean);
...
%center of the group moves. The later seems more in handy.
    ...
    ub = handles.ub;
      ...
      if data_type==1
        ...
        set(handles.dataselection_editText,'String',lb);
        x = data(:,1);
        y = data(:,2);
        theta = data(:,3);
        %[x y theta] = transf_in_familiar_coord(x,y,theta);        
      ...
      file=strcat('Dance',num2str(lb+1),'.dat');
      data_type = handles.data_type;
      if data_type==1
        data=handles.dancedata{lb+1};
        set(handles.dataselection_editText,'String',lb+1);
        x = data(:,1);
        y = data(:,2);
        theta = data(:,3);
        %[x y theta] = transf_in_familiar_coord(x,y,theta);        
      ...
      centerDir_lb = centerGroup2_lb - centerGroup1_lb;
  ...
  for j=1:numberOfNodes
   ...
   index = 1;   
      ...
      thetaFinal = anglerestrict(theta1 - theta2);
      ...
      %PosAngles(lb,j,1:2) = [PosMatrix(j,1),PosMatrix(j,2)]/norm([PosMatrix(j,1),PosMatrix(j,2)]);
  ...
  %} 
   ...
   if answer==1
    ...
    for j=1:numberOfNodes
            ...
            ind = find(squeeze(directions(j,1,:))>frame,1)-1;
  ...
  C_min=0.99;
    for i=max(frame-delay,lb):frame
      ...
      if data_type==1
        ...
        [x2 y2 theta2] = transf_in_familiar_coord(x,y,theta);        

        dim = 2;
        center_group2 = [mean(x2), mean(y2)];
      end
      
      dir1 = x2-x;
      dir2 = y2-y;
      dirGroup = center_group2-center_group;
      theta1 = atan2(dir2, dir1);
      theta2 = atan2(dirGroup(2), dirGroup(1));
      thetaFinal = theta1 - theta2*ones(1, length(theta2));
      cosTheta = cos(thetaFinal);
      sinTheta = sin(thetaFinal);
      ...
      projections(i-max(frame-delay, lb)+1,:,:) = [dir1/norm([dir1, dir2]), dir2/norm([dir1, dir2])];
    ...
    %{
    LeadAndLag_fromCenter = zeros(13,13);
    ...
    for i = max(frame1-delay,lb):frame2
      ...
      theta1 = atan2(dir2, dir1);
      theta2 = atan2(dirGroup(2), dirGroup(1));
      thetaFinal = anglerestrict(theta1 - theta2*ones(1, length(theta2)));
      cosTheta = cos(thetaFinal);
      sinTheta = sin(thetaFinal);
      projections(i-frame1+delay+1,:,:) = [cosTheta, sinTheta];
      %for node = 1:numberOfNodes
       % projections(i-frame1+delay+1,node,:) = [dir1(node)/norm([dir1(node), dir2(node)]), dir2(node)/norm([dir1(node), dir2(node)])];
      %end
    end
    ...
    for i=frame1:frame2
        ...
        for j = 1:numberOfNodes
          for k = 1:numberOfNodes
           ...
           %{
          if (i==330|i==331|i==332|i==333|i==334|i==335|i==336|i==337|i==338|i==339|i==340) & (j==1 & k==9) | (j==9 & k==1)
            i
            j
            k
            squeeze(projections(i-frame1+delay+1,j,:))
            squeeze(projections(i-frame1+delay+1-delay:i-frame1+delay+1,k,:))
            C_tau(i-frame1+1,:)
          end
            %}
...
function moments = MomentsOfInertia(r, start_frame, end_frame, numberOfNodes)
   ...
   for i=1:end_frame-start_frame+1
       for j=1:5
           ...
           switch j
               case 1
                   moments(i,j)=mean(r(i,:));
               case 2
                   moments(i,j)=mean((r(i,:) - mean(r(i,:))).^2);
               case 3
...
X=load('senseGraph.mat', 'adjacencyMatrix');
adj = X.adjacencyMatrix;
ub = size(adj,1);
    ...
    for i=lb:ub %read the data
      ...
      Evector = [Polreal,Polimg];
      %adj = handles.adjacencyMatrix;
      A = squeeze(adj(i,:,:));
      L = Laplacian(A);
      Evector2(:,1) = L*Evector(:,1);
      Evector2(:,2) = L*Evector(:,2);
      
      dotProd = dot(Evector(:,1),Evector2(:,1)) + dot(Evector(:,2),Evector2(:,2));
      W(i) = 1/(2*m*numberOfNodes)*sum(dotProd);
 
      Pol_norm(i) = norm(Pol(i));
    
      r_ic = squeeze(positionMatrix(i,:,:)) - [ones(numberOfNodes,1)*centerMatrix(i,1), ones(numberOfNodes,1)*centerMatrix(i,2)];
      ...
      angular_m(i) = norm(sum_cross)/numberOfNodes;
      
    end
    
    disp('hopefully')
 save('D:\Work stuff\FlockLogic\version 8\W4.mat', 'W')
    ...
    %Pol_norm and angular_m are not dependent on the sensing graph!, but W
    %is
...
switch answer2
   ...
   legend('Mean distance to others', 'Distance to center')
    ...
    case 3
       ...
       for j=1:size(nodes,2)
         dist(i,j,:) = min(distanceMatrix(i, nodes(j),:),2); 
         dist2(i,j) = mean(dist(i,j,:));
       end
    ...
    case 4
        ...
        for j=1:size(nodes,2)
            for i=start_frame:end_frame
                nodes2 = find(adjacencyMatrix(i,nodes(j),:)==1,2);
                distNeigh(j,i) = mean(distanceMatrix(i,j, nodes2));
...
%dot product with center of group
...
for i=start_frame+1:end_frame
          ...
          data_type = handles.data_type;
            ...
            %size(vector)
            %size(squeeze(vector_c(i-start_frame,:)))
            %[vector'; squeeze(vector_c(i-start_frame,:))]
            %dot(vector', squeeze(vector_c(i-start_frame,:)))
    ...
    for o=1:numberOfNodes-1
      ...
      while cnt <= numberOfNodes
        ...
        if ~isempty(queue) 
          ...
          node = queue(1);
          group(i-start_frame+1,cnt2,index) = node;
          
          cnt=cnt+1;
          queue(1) = []; 
          neighbors1 = find(adj(node,:)==1); neighbors2 = find(adj(:,node)==1); 
          int = ismember(neighbors1, neighbors2); neighbors1(int) = [];
          
          neighbors = [neighbors1, neighbors2];
          neighbors = neighbors(find(verify(neighbors)==0));
...
function [theta0, theta, theta2, theta3, theta4, theta_avg, theta_avg2] = direction(handles, hObject)
    ...
    for i=start_frame:end_frame-1
     ...
     if i<=end_frame-delay
        xx = (x(i+delay,:)-x(i,:))/delay;
        yy = (y(i+delay,:)-y(i,:))/delay;
        v2(i,:,:) = [xx', yy']; 
     
     for j=1:numberOfNodes   
           theta2(i-start_frame+1,j) = atan2(v(i,j,2), v(i,j,1));
     end
 ...
 for j=1:numberOfNodes   
   ...
   for j=1:numberOfNodes 
    [theta3(:,j), ignore, ignore, theta_avg(:,j), ignore] = averaging(theta(:,j), delay, 1, 1);
    [theta4(:,j), ignore, ignore, theta_avg2(:,j), ignore] = averaging(theta2(:,j), delay, 1, 1);
   end 
...
for i=1:end_frame-start_frame+1
    for o=1:numberOfNodes
        for s=1:numberOfNodes
            ...
            F(i,o,s) = (1-delta/pi)*1/(1+exp(-2*(dist_norm-1/2)));
            Ftotal(i) = sum(sum(squeeze(F(i,:,:))))/(numberOfNodes*(numberOfNodes-1));
        end
    end
end
  ...
  for j=1:length(nodes) 
   ...
   xx(j,:) = initial;  
      ...
      while norm(old_x - xx(j,:), Inf) > eps1
          ...
          old_x = xx(j,:);
          [xx(j,:), fval(nodes(j)), exitflag(j)] = fmincon(@(x) min_for_sparsity(xx, weights_a, weights_u, q_x, q_y, Y, numberOfNodes, F, delay, lambda, delta), [old_x], A, b, Aeq, beq, zeros(1,numberOfNodes*(delay+1)+2*F), ones(1,numberOfNodes*(delay+1)+2*F), [], options);          
       ...
       for k=1:numberOfNodes
         ...
         if shortestLengths(j,k) > 0
           ...
           if j==1 & k==7
               j
               k
               'shortest length between j and k'
               shortestLengths(j,k)
               'neighbors of 1'
               neighbors
               'starting possibleLength -> either 1 (nodes adjacent!) or total number of neighbors'
               possibleLengths(j,k)
           end
        ...
        while level < shortestLengths(j,k)  
          ...
          for l=1:length(neighbors) 
              if j==1 & k==7
                  j
                  k
                  'neighbor into consideration'
                  neighbors(l)
                  'comparing... sh and sh+level'
                  shortestLengths(j,k)
                  level + shortestLengths(neighbors(l),k)
              end
...
prompt = {'Assesing leadership by one of the 2 options:                                                                        Select1). Node status                                                   Select2). Leadership index                                                             Select 3). Lead and Lag                                                               Select 4). Sparsity connections (Sznaier)                                          Select 5). Other centrality measures'};
...
switch option    
    case 1
        ...
        figure(1)
        for j=1:length(nodes)
            plot(start_frame:end_frame, nodeStatus(start_frame:end_frame, nodes(:,j)), 'Color', colors(j,:))
            hold on;
            j
            disp('average node status & std of node status')
            nodeStatus_mean(j) = mean(nodeStatus(start_frame:end_frame, nodes(:,j)))
            nodeStatus_std(j) = mean(nodeStatus(start_frame:end_frame, nodes(:,j)))
        end
        
        %legend('node 1', 'node 2', 'node 3', 'node 4', 'node 5', 'node 6', 'node 7', 'node 8', 'node 9', 'node 10', 'node 11', 'node 12', 'node 13');
        
        for i=start_frame:end_frame
            [maxNodeStatus, indexNode] = max(nodeStatus(i,:));
            leader(i) = indexNode;
        end
        ...
        figure(2)
        plot(1:numberOfNodes, nodeStatus_mean, 'x', 'MarkerSize', 12)
        
        [nodeStatus_IG, nodeStatus_avg2_IG, degree_in, degree_out] = nodeStatusIG(handles, hObject, start_frame, end_frame);
    ...
    case 5
        ...
        X=load('180_r05.mat', 'adjacencyMatrix');
        adj = X.adjacencyMatrix;
        size(adj)    
        ...
        switch option2
            ...
            case 6
                ...
                degree_out_mean = mean(degree_out);
                degree_out_std = std(degree_out);
                'Sort out-degree mean'
                [ignore, Ind_sorted] = sort(degree_out_mean);
                Ind_sorted
                'Sort degree std'
                [ignore, Ind_sorted] = sort(degree_out_std);
                Ind_sorted
        end
end
...
for i=start_frame:end_frame
 ...
 for R = array
  cnt = cnt+1;
  [V_cum(cnt),S_cum(cnt)] = alphavol(squeeze(positionMatrix(i,:,:)),R);
 end
...
prompt = {'Select what to do:                                                                               Select 1). fit R                                                                                                                                    Select 2) compare node status among graphs                                                                                                                                                          Select 3). Create huge matrix for ML                                                                                                                                                      Select 4). Plot direction of center                                                                                                                                               Select 5). compute inertia matrix and principal moments of inertia                                                                                                                                                          Select 6). Compute density'};    
...
switch Misc_choice
    case 1
r_0 = fit_R(handles, hObject)
    case 2
compare_graph_properties(hObject, handles, start_frame, end_frame)
    case 3
group_properties(hObject,handles)        
    case 4
center_dir(hObject, handles, 30, start_frame, end_frame)
    case 5
[positionMatrix, center, numberOfNodes] = Read_coordinatesAndCenter(start_frame, end_frame, handles, hObject);
[I, V, D] = inertia_matrix(positionMatrix, handles, hObject);
...
for adj_graph=1:3  
    ...
    switch adj_graph
        case 1
            disp('180 degrees, R=0.3')
            adj = choose_graph(hObject, handles,2);

        case 2
            disp('180 degrees, R=0.4')
            adj = choose_graph(hObject, handles,3);
            
        case 3
            disp('180 degrees, R=0.5')
            adj = choose_graph(hObject, handles,4);
            %end_frame = 916;      
    end
...
%{  
  ...
  disp('node status and distance to center correlated')
%  [xcf, lags, bounds] = xcorr(nodeStatus(1:end_frame-start_frame+1, node)-mean(nodeStatus(1:end_frame-start_frame+1, node)), dist_to_center(1:end_frame-start_frame+1, node), 'coeff');
  %xcf'
  %bounds'
  ...
  disp('node status and distance to center-axis')
  %[xcf, lags, bounds] = xcorr(nodeStatus(1:end_frame-start_frame-19,node), Distance3(1:end_frame-start_frame-19, node), 'coeff');
  %xcf'
  %lags'
  %bounds'
  ...
  disp('node status and abs value of distance to center-axis correlated')
  %[xcf, lags, bounds] = crosscorr(nodeStatus(1:end_frame-start_frame-19, node), distance(1:end_frame-start_frame-19, node));
  %xcf'
  %lags'
  %bounds'
 ...
 legend('node status SG', 'node status IG')
  ...
  disp('node status SG and node status IG correlated')
  %[xcf, lags, bounds] = crosscorr(nodeStatus(1:end_frame-start_frame, node), nodeStatus_IG(1:end_frame-start_frame, node));
  %xcf'
  %lags'
  %bounds'
  [c p] = corrcoef(nodeStatus_smooth2(1:end_frame-start_frame), nodeStatus_IG_smooth2(1:end_frame-start_frame));
 ...
 disp('node status IG and distance to center correlated')
 %[xcf, lags, bounds] = crosscorr(nodeStatus_IG(start_frame:end_frame, node), dist_to_center(1:end_frame-start_frame+1, node));
  %xcf'
  %lags'
  %bounds'
 ...
 disp('node status IG and distance from center-axis correlated')
  %[xcf, lags, bounds] = crosscorr(nodeStatus_IG(start_frame:end_frame-20, node), Distance3(1:end_frame-start_frame-19, node));
  %xcf'
  %lags'
  %bounds'
 ...
 legend('node status IG', 'abs distance')
  ...
  disp('node status IG and absolute value of distance to center-axis correlated')
  %[xcf, lags, bounds] = crosscorr(nodeStatus_IG(start_frame:end_frame-20, node), distance(1:end_frame-start_frame-19, node));
  %xcf'
  %lags'
  %bounds'
  ...
  for s = 1:size(dist_to_center2,1)
      ...
      corr_row = [corr_row, s*ones(1,dim2)]; 
      corr_col = [corr_col, 1:dim2];
  end
 ...
 title('How many trials per bin for d to center(point) - front, middle, back')
  ...
  %}
      ...
      [nodeStatus, nodeStatus_avg] = compute_nodeStatus(hObject, handles, adj, start_frame, end_frame)
      [dist_to_center, avg_toCenter] = distToCenter(handles, hObject, start_frame, end_frame, numberOfNodes,0)
      [Distance2, Distance3, distance, leader, backer, N_leader, N_backer] = LeaderBacker(hObject, handles, start_frame, end_frame,0)
      ...
      figure(1)
      plot(1:numberOfNodes, nodeStatus_avg, 'x', 'Color', 'b', 'MarkerSize', 14);
      hold on
      ...
      disp('average node status and average distance to center correlation')
      ...
      disp('average node status and average distance to center-axis correlation')
     ...
     % bounds'
      ...
      hold on
      plot(1:numberOfNodes, nodeStatus_avg2_IG, 'x', 'Color', 'g', 'MarkerSize', 14)
      ...
      disp('average node status and node status in interaction graph correlation')
     % [xcf, lags, bounds] = crosscorr(nodeStatus_avg, nodeStatus_avg2_IG);
     % xcf'
     % bounds'
      [c p] = corrcoef(nodeStatus_avg, nodeStatus_avg2_IG);
      c
      p
      
      disp('abs value of average distance from center-axis and node status of interaction graph correlation')
      %[xcf, lags, bounds] = crosscorr(distance_avg, nodeStatus_avg2_IG);
      %xcf'
      %bounds'
      ...
      figure(4)
       plot(1:numberOfNodes, nodeStatus_avg, 'x', 'Color', 'b', 'MarkerSize', 14)
       hold on
       plot(1:numberOfNodes, nodeStatus_avg2_IG, 'x', 'Color', 'r', 'MarkerSize', 14)
       ...
       legend('average node status SG', 'average node status IG')
        ...
        plot(1:numberOfNodes, avg_toCenter, 'x', 'Color', 'r', 'MarkerSize', 14)
       hold on
        plot(1:numberOfNodes, nodeStatus_avg2_IG, 'x', 'Color', 'b', 'MarkerSize', 14)
        title('For each node, average distance to center and node status IG')
        xlabel('node')
        legend('average distance to center', 'node status IG')
        
        disp('average distance to center and node status of interaction graph correlated')
     ...
     % bounds'
      ...
      disp('average distance from center-axis and node status of interaction graph correlated')
